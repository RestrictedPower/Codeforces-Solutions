...
/*
    Solution Created: 17:50:45 30/12/2020
...
public class Main {
	public static void solve(Reader in, Writer out) {
		...
		while(t-->0){
			...
			for(int i = 0; i<n; i++) {
				if(sz[i]<=1) continue;
				times.put(cost[i], times.getOrDefault(cost[i], 0)+ sz[i]-1);
			}
			out.print(sum+" ");
			for(int i = 0; i<n-2; i++) {
				if(!times.isEmpty()) {
					int last = times.lastKey();
					int left = times.get(last);
					if(left==1) times.remove(last);
					else times.put(last, left-1);
					sum+=last;
				}
				out.print(sum+" ");
			}
