...
/*
    Solution Created: 16:55:01 31/03/2020
...
public class Main {
	public static void solve(Reader in, Writer out) {
		...
		while(t-->0) {
			...
			int ymax = in.nextInt();
			boolean can = true;
			if(!(xmin<=x+mx&&x+mx<=xmax)) can = false;
			if(!(ymin<=y+my&&y+my<=ymax)) can = false;
			if(xmax-xmin==0&&(a[1]>0||a[0]>0)) can = false;
			if(ymax-ymin==0&&(a[3]>0||a[2]>0)) can = false;
