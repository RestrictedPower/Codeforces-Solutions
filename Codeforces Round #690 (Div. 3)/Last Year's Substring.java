...
/*
    Solution Created: 00:51:27 16/12/2020
...
public class Main {
	public static void solve(Reader in, Writer out) {
		...
		while(t-->0){
			int n = in.nextInt();
			String s = in.next();
			out.println(can(s)?"YES":"NO");
		}
	}
	static boolean can(String s){
		if(s.startsWith("2020")||s.endsWith("2020")) return true;
		if(s.startsWith("2")&&s.endsWith("020")) return true;
		if(s.startsWith("20")&&s.endsWith("20")) return true;
		if(s.startsWith("202")&&s.endsWith("0")) return true;
		return false;
	}
