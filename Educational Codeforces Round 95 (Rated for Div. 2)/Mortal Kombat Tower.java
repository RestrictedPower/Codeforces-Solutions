...
/*
    Solution Created: 18:23:41 14/09/2020
...
public class Main {
	public static void solve(Reader in, Writer out) {
		...
		while(t-->0) {
			...
			dp[0][0] = a[0];
			if(n!=1) dp[1][0] = a[0] + a[1];
			for(int i = 0; i<n; i++) {
				if(i+1<n) dp[i+1][0] = Math.min(dp[i+1][0], dp[i][1] + a[i+1]);
				if(i+1<n) dp[i+1][1] = Math.min(dp[i+1][1], dp[i][0]);
				if(i+2<n) dp[i+2][0] = Math.min(dp[i+2][0], dp[i][1] + a[i+1]+ a[i+2]);
				if(i+2<n) dp[i+2][1] = Math.min(dp[i+2][1], dp[i][0]);
			}
