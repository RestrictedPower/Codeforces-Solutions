...
/*
    Solution Created: 22:47:15 14/09/2020
...
public class Main {
	...
	static TreeMap<Integer,Integer> differencies = new TreeMap<Integer,Integer>();
	public static  void remDiff(int d) {
		if(differencies.containsKey(d)) {
			if(differencies.get(d)==1) differencies.remove(d);
			else differencies.put(d, differencies.get(d)-1);
		}
	}
	public static void addDif(int d) {
		differencies.put(d, differencies.getOrDefault(d, 0)+1);
	}
	public static void add(int i) {
		int h = s.higher(i)==null?-1:s.higher(i), l = s.lower(i)==null?-1:s.lower(i);
		if(l!=-1) {
			int nextL = s.higher(l)==null?-1:s.higher(l);
			if(nextL!=-1) remDiff(nextL-l);
			addDif(i-l);
		}
		if(h!=-1) { 
			int prevH = s.lower(h)==null?-1:s.lower(h);
			if(prevH!=l && prevH!=-1) remDiff(h-prevH);
			addDif(h-i);
		}
		s.add(i);
	}
	public static void rem(int i) {
		...
		if(l!=-1) {
			remDiff(i-l);
			int nextL = s.higher(l)==null?-1:s.higher(l);
			if(nextL!=-1) addDif(nextL-l);
		}
		if(h!=-1) {
			remDiff(h-i);
			int prevH = s.lower(h)==null?-1:s.lower(h);
			if(prevH!=l && prevH!=-1) addDif(h-prevH);
		}
	}
	static int query() {
		if(s.size()<=2) return 0;
		return s.last()-s.first() - differencies.lastKey();
	}
	public static void solve(Reader in, Writer out) {
		n = in.nextInt();
		q = in.nextInt();
		int[] a = in.na(n);
		for(Integer i : a) add(i);
		out.println(query());
		for(int i = 0; i<q; i++) {
			int op = in.nextInt(), v =in.nextInt();
			if(op==0) rem(v);
			else add(v);
			out.println(query());
		}
