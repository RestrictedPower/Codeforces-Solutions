...
/*
    Solution Created: 18:31:33 15/08/2020
...
public class Main {
	public static void solve(Reader in, Writer out) {
		...
		while(T-->0) {
			...
			int[] rA = in.na(r), gA = in.na(g), bA = in.na(b);
			Util.sortArray(rA);
			Util.sortArray(gA);
			Util.sortArray(bA);
			Util.reverse(rA);
			...
			for(int i = 0; i<=r; i++) for(int j = 0; j<=g; j++) for(int k = 0; k<=b; k++) {
				if(i>0&&j>0) dp[i][j][k] = Math.max(dp[i][j][k], dp[i-1][j-1][k] + rA[i-1]*gA[j-1]);
				if(j>0&&k>0) dp[i][j][k] = Math.max(dp[i][j][k], dp[i][j-1][k-1] + gA[j-1]*bA[k-1]);
				if(i>0&&k>0) dp[i][j][k] = Math.max(dp[i][j][k], dp[i-1][j][k-1] + rA[i-1]*bA[k-1]);
