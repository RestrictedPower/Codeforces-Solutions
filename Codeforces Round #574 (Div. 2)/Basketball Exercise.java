...
/*
    Solution Created: 04:50:27 18/07/2019
    Custom Competitive programming helper.
*/
public class Main {
	public static void solve(Reader in, Writer out) {
		int n = in.nextInt();
		long[] a = new long[n+1],b = new long[n+1],dpa = new long[n+1], dpb = new long[n+1];
		for(int i = 1; i<=n; i++) a[i] = in.nextInt();
		for(int j = 1; j<=n; j++) b[j] = in.nextInt();
		for(int i = 1; i<=n; i++) {
			dpa[i] = Math.max(dpb[i-1]+a[i], dpa[i-1]);
			dpb[i] = Math.max(dpa[i-1]+b[i], dpb[i-1]);
		}
		out.println(Math.max(dpa[n], dpb[n]));
	}
