...
public class Main {
	...
	public static void solve(Reader in, Writer out) {
		...
		int pL = in.nextInt();
		int[] p = new int[pL];
		for(int i = 0; i<pL; i++) p[i] = in.nextInt();
		int s = p[0], t = p[pL-1];
		int[] shortestPath = new int[n+1];
		bfs(shortestPath,t,aR);
		int minRebuilds = 0,maxRebuilds = 0;
		
		int curS = shortestPath[s];
		for(int i = 1; i<pL; i++) {
			if(curS<=shortestPath[p[i]]) minRebuilds++;
			curS = shortestPath[p[i]];
		}
		for(int i = 1; i<pL-1; i++) {
			curS = shortestPath[p[i]];
			if(curS>=shortestPath[p[i-1]]) {
				maxRebuilds++;
				continue;
			}
			int rebuilds = 0;
			for(Integer j : a[p[i-1]]) {
				if(shortestPath[j]==curS) rebuilds++;
			}
			if(rebuilds>1) maxRebuilds++;
		}
		out.println(minRebuilds+" "+maxRebuilds);
 	}
	public static void bfs(int[] a, int cur, ArrayList<Integer> con[]) {
		a[cur] = 0;
