...
/*
    Solution Created: 17:22:42 01/07/2020
...
public class Main {
	public static void solve(Reader in, Writer out) {
		int t = in.nextInt();
		while(t-->0) {
			out.println(ans(in.nextLong(), in.nextLong(), in.nextLong(), in.nextLong())?"Yes":"No");
		}
	}
	public static boolean ans(long v, long c, long f, long s) {
		if(f==0&&s==0) return true;
		if(v==0&&c==0) return false;
		long mn = Math.min(v, c);
