...
/*
    Solution Created: 07:46:25 21/01/2020
...
public class Main {
	public static void solve(Reader in, Writer out) {
		...
		while(t-->0) {
			int n = in.nextInt();
			Pin[] a = new Pin[n];
			for(int i = 0; i<n; i++) a[i] = new Pin(in.next(), i);
			Arrays.sort(a);
			HashSet<String> used = new HashSet<String>();
			for(Pin p : a)used.add(p.pin);
			int ans = 0;
			for(int i = 0; i<n-1; i++) {
				if(a[i].pin.equals(a[i+1].pin)) {
					ans++;
					String s = a[i].pin;
					int id = 0;
			...
			Pin[] b = new Pin[n];
			for(int i = 0; i<n; i++) b[a[i].id] = a[i];
			out.println(ans);
			for(int i = 0; i<n; i++) out.println(b[i].pin);
		}
	}
	static String next(String s, int id) {
		int idx = 0;
		for(int i = 0; i<4; i++) {
			for(int j = 0; j<10; j++) {
				if(id != idx++) continue;
				return replace(s, i, (char)('0'+j));
			}
		}
		return null;
	}
	...
	static class Pin implements Comparable<Pin>{
		String pin;
		int id;
		public Pin(String pin, int id) {
			this.pin = pin;
			this.id = id;
		}
		@Override
		public int compareTo(Pin arg0) {
			return this.pin.compareTo(arg0.pin);
		}
	}
