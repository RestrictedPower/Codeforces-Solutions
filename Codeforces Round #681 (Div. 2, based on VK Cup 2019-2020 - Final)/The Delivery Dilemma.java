...
/*
    Solution Created: 17:35:29 02/11/2020
...
public class Main {
	public static void solve(Reader in, Writer out) {
		...
		while(t-->0) {
			int n = in.nextInt();
			long[] c = in.nl(n), d = in.nl(n);
			Pair[] a = new Pair[n];
			for(int i = 0; i<n; i++) a[i] = new Pair(c[i], d[i]);
			Util.sortArray(a);
			long[] suf = new long[n];
			suf[n-1] = a[n-1].b;
			for(int i =n-2; i>=0; i--) suf[i] = suf[i+1]+a[i].b;
			long ans = Math.min(suf[0], a[n-1].a);
			
			for(int i = 0; i<n-1; i++) {
				ans = Math.min(ans, Math.max(suf[i+1], a[i].a));
			}
			out.println(ans);
		}
	}
