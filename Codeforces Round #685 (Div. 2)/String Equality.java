...
/*
    Solution Created: 17:25:22 21/11/2020
...
public class Main {
	public static void solve(Reader in, Writer out) {
		...
		while(t-->0){
			...
			for(char c : b) f2[c-'a']++;
			for(int i = 0; i<26; i++) {
				int maxTake = Math.min(f1[i], f2[i]);
				f1[i]-=maxTake;
				f2[i]-=maxTake;
			}
			boolean can = true;
			for(int i = 0; i<26; i++){
				if(f1[i]%k!=0) can = false;
				if(f2[i]%k!=0) can = false;
				f1[i]/=k;
				f2[i]/=k;
			}
			for(int i = 25; i>=0 && can; i--){
				for(int j = 25; j>=i; j--){
					if(f2[j]>0){
						int maxTake = Math.min(f2[j], f1[i]);
						f1[i]-=maxTake;
						f2[j]-=maxTake;
					}
				}
			...
			for(int i = 0; i<26; i++) if(f2[i]>0) can = false;
