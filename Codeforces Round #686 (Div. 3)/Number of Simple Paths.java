...
/*
    Solution Created: 07:34:35 28/11/2020
...
public class Main {
	...
	static HashSet<Integer> cyc;
	static Stack<Integer> cur;
	...
	public static void solve(Reader in, Writer out) {
		...
		while(t-->0){
			...
			cyc = new HashSet<Integer>();
			cur = new Stack<Integer>();
			findCycle(0, -1);
			long cSize = cyc.size();
			long ans = (cSize-1)*cSize, totalEndPoints = n-cSize;
			for(int i : cyc) {
				long endPointCount = dfs(i, -1)-1;
				if(endPointCount<=0) continue;
				long otherEndpoints = totalEndPoints-endPointCount;
				ans += otherEndpoints*endPointCount; //This should be multiplied by 2 but we double count anyways
				ans += endPointCount*(endPointCount-1)/2;
				ans += 2*(endPointCount)*(cSize-1) + endPointCount;
			}
			out.println(ans);
		}
	}
	public static int dfs(int i, int prev) {
		int gain = 1;
		for(Integer nx : adj[i]) {
			if(nx != prev && (!cyc.contains(nx))) gain += dfs(nx, i);
		}
		return gain;
	}
	public static void findCycle(int c, int prev) {
		if(found) return;
		visited[c] = true;
		cur.add(c);
		for(Integer i : adj[c]) {
			if(i==prev || found) continue;
			if(visited[i]) {
				found = true;
				while(cur.peek()!=i) {
					cyc.add(cur.pop());
