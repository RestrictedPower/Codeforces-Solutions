...
/*
    Solution Created: 06:28:59 25/11/2020
...
public class Main {
	public static void solve(Reader in, Writer out) {
		...
		while(t-->0){
			long n = in.nextLong();
			ArrayList<Long> ans = maxBreak(n,1);
			out.println(ans.size());
			for(Long i : ans) out.print(i+" ");
			out.println();
		}
	}
	public static ArrayList<Long> maxBreak(long v, long prev){
		ArrayList<Long> ans = new ArrayList<>();
		ans.add(v);
		for(long i = 2; i*i<=v; i++){
			if(v%i!=0 || (v/i)%i!=0 || i%prev!=0) continue;
			ArrayList<Long> get = maxBreak(v/i,i);
			if(get.size()+1>ans.size()){
				get.add(0, i);
				ans = get;
			}
		}
