...
/*
    Solution Created: 23:32:35 24/11/2020
...
public class Main {
	public static void solve(Reader in, Writer out) {
		...
		while(t-->0){
			...
			for(int i  : a) if(u.isEmpty()|| u.get(u.size()-1)!=i) u.add(i);
			...
			for(int i : u){
				int between = f[i]-1;
				if(u.get(0)!=i) between++;
				if(u.get(u.size()-1)!=i) between++;
				ans = Math.min(ans, between);
			}
			out.println(ans);
		}
	}
