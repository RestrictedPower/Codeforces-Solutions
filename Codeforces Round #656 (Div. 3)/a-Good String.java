...
/*
    Solution Created: 15:24:18 19/09/2020
...
public class Main {
	public static void solve(Reader in, Writer out) {
		...
		while(t-->0) {
			int n = in.nextInt();
			char[] a = in.nca();
			out.println(ans(a, 0, n-1, 'a'));
		}
	}
	public static int ans(char[] a, int l, int r, char cur) {
		if(l==r) return (a[l]==cur?0:1);
		int m = (l+r)/2;
		int countA = 0, countB = 0;
		for(int i = l; i<=m; i++) countA+=(a[i]==cur?0:1);
		for(int i = m+1; i<=r; i++) countB+=(a[i]==cur?0:1);
		return Math.min(countA + ans(a, m+1, r,(char)(cur+1)), countB + ans(a, l, m,(char)(cur+1)));
	}
