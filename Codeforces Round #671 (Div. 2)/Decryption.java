...
/*
    Solution Created: 00:37:59 20/09/2020
...
public class Main {
	public static void solve(Reader in, Writer out) {
		...
		while(t-->0) {
			int n = in.nextInt();
			HashSet<Integer> divs = getDivisors(n);
			ArrayList<Integer> primes = new ArrayList<Integer>();
			for(Integer i : divs)if(Util.isPrime(i))primes.add(i);
			for(Integer i : primes) divs.remove(i);
			int pn = primes.size();
			int[] con = new int[pn];
			for(int i = 0; i<pn-1; i++) {
				int v1 = primes.get(i), v2 = primes.get(i+1);
				int m = v1*v2;
				if(divs.contains(m)) {
					con[i] = m;
					divs.remove(m);
			...
			int v1 = primes.get(pn-1), v2 = primes.get(0);
			int m = v1*v2;
			if(divs.contains(m)) {
				con[pn-1] = m;
				divs.remove(m);
				...
				int found = -1;
				for(Integer i : divs) if(i%(m)==0) found = i;
				if(found!=-1) divs.remove(found);
				con[pn-1] = found;
			}
			
			ArrayList<Integer> vals[] = new ArrayList[pn];
			for(int i = 0; i<pn; i++) vals[i] = new ArrayList<Integer>();
			for(Integer i : divs) {
				for(int j = 0; j<pn; j++) {
					if(i%primes.get(j)==0) {
						vals[j].add(i);
			...
			for(int i = 0; i<pn; i++) {
				...
				for(Integer j : vals[i]) out.print(j+" ");
				out.print((con[i]==-1?"":con[i])+" ");
			}
			out.println();
			out.println(con[pn-1]==-1?1:0);
		} 
	}
