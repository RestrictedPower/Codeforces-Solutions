...
/*
    Solution Created: 12:06:23 16/04/2020
...
public class Main {
	...
	public static void solve(Reader in, Writer out) {
		...
		while(t-->0) {
			...
			Pair[] a = new Pair[n];
			int idx = 0;
			for(int i = 0; i<r; i++) a[idx++] = new Pair(rw[i], 0);
			for(int i = 0; i<g; i++) a[idx++] = new Pair(gw[i], 1);
			for(int i = 0; i<b; i++) a[idx++] = new Pair(bw[i], 2);
			Util.sortArray(a);
			...
			for(int i = 1; i<n-1; i++) {
				...
				for(int col1=0; col1<3; col1++) {
					...
					for(int col3=0; col3<3; col3++) {
						if(col3==col2||col3==col1) continue;
						int[] first = cA(col1);
						int[] last = cA(col3);
						int pos1 = Util.upperBound(first, a[i].w)-1;
						int pos3 = Util.lowerBound(last, a[i].w);
						if(pos1==-1 || pos3 == last.length) continue;
						long x = first[pos1];
						long y = a[i].w;
						long z = last[pos3];
						ans = Math.min(ans, comp(x,y,z));
					}
				}
	...
	public static long s(long v) {
		return v*v;
	}
	public static long comp(long x, long y,long z) {
		return s(x-y)+s(y-z)+s(x-z);
	}
