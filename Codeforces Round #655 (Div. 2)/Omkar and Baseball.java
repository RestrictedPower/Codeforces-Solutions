...
/*
    Solution Created: 21:53:11 14/07/2020
...
public class Main {
	public static void solve(Reader in, Writer out) {
		...
		while(t-->0) {
			...
			int inPos = 0;
			int l = -1, r = -1;
			for(int i = 0; i<n; i++) if(a[i]==i+1) inPos++;
			else {
				if(l==-1) l = i;
				r = i;
			}
			boolean isBad = false;
			if(inPos!=n)
			for(int i = l; i<=r; i++) if(a[i]==i+1) isBad = true;
			if(inPos==n) out.println(0);
			else if(inPos==0||(!isBad)) out.println(1);
			else out.println(2);
		}
	}
