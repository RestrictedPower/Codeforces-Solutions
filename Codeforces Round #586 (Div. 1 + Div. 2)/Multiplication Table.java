...
/*
    Solution Created: 19:37:48 18/09/2019
...
public class Main {
	public static void solve(Reader in, Writer out) {
		...
		out.println(Math.sqrt(m[0+2][1+2]*m[0+2][2+2]/m[1+2][2+2]));
		int[] ans = new int[n];
		for(int i = 0; i<n-2; i++) {
			out.println(ans[i]=(int)Math.sqrt(m[0+i][1+i]*m[0+i][2+i]/m[1+i][2+i]));
		}
		out.println((int)(m[n-2][0]/ans[0]));
		out.println((int)(m[n-1][0]/ans[0]));*/
		for(int i = 0; i<n; i++) for(int j = 0; j<n; j++) m[i][j] = in.nextInt();
		long a0 = (long) Math.sqrt(m[0][1]*m[0][2]/m[1][2]);
		out.print(a0+" ");
		for(int i = 1; i<n; i++) {
			out.print((int)(m[0][i]/a0)+" ");
		}
	}
