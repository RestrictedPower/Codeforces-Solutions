...
/*
    Solution Created: 16:59:19 24/02/2020
...
public class Main {
	public static void solve(Reader in, Writer out) {
		...
		while (t-- > 0) {
			...
			int[] ans = new int[26];
			for (int i = 0; i < m; i++) for (int j = 0; j < 26; j++) ans[j] += f[p[i]-1][j];
			
			for(int i = 0; i<n; i++) ans[a[i]-'a']++;
			for(int i = 0; i<26; i++) out.print(ans[i]+" ");
			out.println();
		}
