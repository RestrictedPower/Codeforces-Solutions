...
/*
    Solution Created: 22:08:43 24/02/2020
...
public class Main {
	public static void solve(Reader in, Writer out) {
		...
		while(t-->0) {
			...
			int Adif = Integer.MAX_VALUE;
			int A=0,B=0,C=0;
			for(int a1 = 1; a1<=11000; a1++) {
				for(int b1 = a1; b1<=11000; b1+=a1) {
					for(int c1 = b1; c1<=11000; c1+=b1) {
						int dif = Math.abs(a-a1)+Math.abs(b-b1)+Math.abs(c-c1);
						if(dif<Adif) {
							Adif = dif;
							A = a1;
							B = b1;
