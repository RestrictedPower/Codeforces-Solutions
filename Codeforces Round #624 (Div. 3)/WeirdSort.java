...
/*
    Solution Created: 16:45:07 24/02/2020
...
public class Main {
	public static void solve(Reader in, Writer out) {
		...
		while (t-- > 0) {
			...
			for(int i = 0; i<m; i++) s.add(p[i]-1);
			HashSet<Integer> needSwap = new HashSet<Integer>();
			for (int i = 0; i < n - 1; i++) {
			...
			for(Integer i : needSwap) if(!s.contains(i)) can = false;
