...
/*
    Solution Created: 18:10:30 30/11/2020
...
public class Main {
	public static void solve(Reader in, Writer out) {
		...
		while(t-->0){
			...
			while(true){
				int firstNotSorted = n;
				for(int i = 1; i<n; i++) if(a[i-1]>a[i]) {
					firstNotSorted = i;
					break;
				}
				if(firstNotSorted==n) break;
				
				int pos = findSmallerPos(a, x);
				if(pos>firstNotSorted){
					can = false;
					break;
				}
//				Util.dbg(pos+" "+firstNotSorted);
				for(int i = pos; i<=firstNotSorted && a[i]>x; i++){
					int tmp = a[i];
					a[i] = x;
					x = tmp;
					moves++;
			...
			out.println(can?moves:-1);
		}
	}
	public static int findSmallerPos(int[] a, int x){
