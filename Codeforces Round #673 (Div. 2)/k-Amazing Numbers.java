...
/*
    Solution Created: 18:34:49 27/09/2020
...
public class Main {
	public static void solve(Reader in, Writer out) {
		...
		while(t-->0) {
			int n = in.nextInt();
			int[] a = in.na(n);
			int[] maxDif = new int[n+1];
			HashMap<Integer, Integer> lastPos = new HashMap<Integer, Integer>();
			for(int i = 1; i<=n; i++) lastPos.put(i, -1);
			for(int i = 0; i<n; i++) {
				int lp = lastPos.get(a[i]);
				maxDif[a[i]] = Math.max(maxDif[a[i]], i-lp);
				lastPos.put(a[i], i);
			}
			
			for(int i = 1; i<=n; i++) {
				int lp = lastPos.get(i);
				maxDif[i] = Math.max(maxDif[i], n-lp);
			}
			int[] ans = new int[n+1];
			Arrays.fill(ans, -1);
			for(int i = 1; i<=n; i++) {
