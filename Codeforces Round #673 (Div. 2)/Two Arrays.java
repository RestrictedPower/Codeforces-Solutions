...
/*
    Solution Created: 18:21:52 27/09/2020
...
public class Main {
	public static void solve(Reader in, Writer out) {
		...
		while(t-->0) {
			...
			int[] a = in.na(n);
			HashSet<Integer> f = new HashSet<Integer>(), s = new HashSet<Integer>();
			for(Integer i : a) {
				if(!f.contains(T-i)) f.add(i);
				else s.add(i);
			}
			HashMap<Integer, Boolean> last = new HashMap<Integer, Boolean>();
			for(int i = 0; i<n; i++) {
				if(f.contains(a[i]) && s.contains(a[i])) {
					boolean first = false;
					if(last.containsKey(a[i])) first = last.get(a[i]);
					out.print((first?1:0)+" ");
					last.put(a[i], !first);
				}else if(f.contains(a[i])) out.print(1+" ");
				else out.print(0+" ");
			}
			out.println();
		}
