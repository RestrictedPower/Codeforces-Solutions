...
/*
    Solution Created: 17:35:54 23/03/2020
...
public class Main {
	public static void solve(Reader in, Writer out) {
		...
		for(int i = 1; i<n; i++) ans.add('U');
		for(int i = 1; i<m; i++) ans.add('L');
		boolean l = false;
		for(int i = 0; i<n; i++) {
			for(int j = 1; j<m; j++) ans.add(l?'L':'R');
			l = !l;
			if(i!=n-1)ans.add('D');
		}
		out.println(ans.size());
		for(Character c : ans) out.print(c);
	}
