...
/*
    Solution Created: 17:07:03 23/03/2020
	...
	public static void solve(Reader in, Writer out) {
		...
		while(t-->0) {
			...
			for(int i = 0; i<n; i++) {
				...
				if(!ok && notOk==-1) notOk = i; 
			}
			int nwantedId = -1;
			for(int i = 0; i<n; i++) if(!taken[i]) nwantedId = i;
			if(notOk!=-1 && nwantedId!=-1) {
				out.println("IMPROVE");
				out.println((notOk+1)+" "+(nwantedId+1));
			}else out.println("OPTIMAL");
		}
	}
