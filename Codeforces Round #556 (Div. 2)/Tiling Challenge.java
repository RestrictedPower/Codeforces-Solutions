...
public class Main {
	public static void main(String[] args) {
		...
		System.out.println(can(a, n)?"YES":"NO");
				
	}
	public static boolean can(boolean[][] a,int n) {
		for(int i = 0; i<n; i++) {
			for(int j = 0; j<n; j++) {
				if(!a[i][j]) {
					if(!isOK(a, i+1, j-1, n)) return false;
					if(!isOK(a, i+1, j, n)) return false;
					if(!isOK(a, i+1, j+1, n)) return false;
					if(!isOK(a, i+2, j, n)) return false;
					a[i][j] = true;
					a[i+1][j-1] = true;
					a[i+1][j] = true;
					a[i+1][j+1] = true;
	...
	public static boolean isOK(boolean[][] a, int i,int j,int n) {
