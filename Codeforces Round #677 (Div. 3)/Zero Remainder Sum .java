...
/*
    Solution Created: 03:27:16 21/10/2020
...
public class Main {
	public static void solve(Reader in, Writer out) {
		...
		for(int ll = 0; ll<n; ll++) {
			int[] a = in.na(m);
			int maxElement = 0;
			for(int i : a) maxElement = Math.max(i, maxElement); 
			int maxUse = m/2;
			int maxSum = maxElement*maxUse;
			int[] dpp = new int[maxSum+1];
			Arrays.fill(dpp, 100);
			dpp[0] = 0;
			for(int num : a) {
				for(int i = maxSum; i>=0; i--) {
					if(dpp[i]!=100 && i+num<=maxSum) {
						dpp[i+num] = Math.min(dpp[i]+1, dpp[i+num]);
					}
				}
			}
			int[] maxForThis = new int[k];
			Arrays.fill(maxForThis, -100000);
			for(int i = 0; i<=maxSum; i++) {
				if(dpp[i]<=maxUse) maxForThis[i%k] = Math.max(maxForThis[i%k], i);
			}
			dp[ll] = maxForThis;
		}
		for(int i = 1; i<n; i++) {
			int[] curState = new int[k];
			Arrays.fill(curState, -100000);
			for(int j = 0; j<k; j++) {
				for(int l = 0; l<k; l++) {
					curState[(j+l)%k] = Math.max(curState[(j+l)%k], dp[i-1][j] + dp[i][l]);
				}
			}
			dp[i] = curState;
		}
