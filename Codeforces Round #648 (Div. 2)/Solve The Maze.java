...
/*
    Solution Created: 18:57:08 07/06/2020
    Custom Competitive programming helper.
*/

public class Main {
	static int[][] g;//0 = empty, 1 = wall, 2 = good
	static boolean[][] vis;
	static int n,m, countGood;
	static boolean can;
	public static void solve(Reader in, Writer out) {
		...
		while(t-->0) {
			...
			countGood = 0;
			can = true;
			ArrayList<Pair> walls = new ArrayList<Main.Pair>();
			for(int i = 0; i<n; i++) {
				char[] s = in.nca();
				for(int j = 0; j<m; j++) {
					if(s[j]=='.') g[i][j] = 0;
						...
						countGood++;
						g[i][j] = 2;
					}else if(s[j]=='B') walls.add(new Pair(i, j));
				}
			}
			for(Pair p : walls) putWallAround(p.x, p.y);
			dfs(n-1, m-1);
			if(countGood==0 && can) out.println("Yes");
			else out.println("No");
		}
	}
	public static void dfs(int i, int j) {
		...
		vis[i][j] = true;
		if(g[i][j]==1) return;
		if(g[i][j]==2) countGood--;
		dfs(i+1, j);
		dfs(i-1, j);
		dfs(i, j+1);
		dfs(i, j-1);
	}
	public static void putWallAround(int x, int y) {
		putWall(x+1, y);
		putWall(x-1, y);
		putWall(x, y+1);
		putWall(x, y-1);
	}
	public static void putWall(int x, int y) {
		if(!isIn(x, y)) return;
		if(g[x][y]==2) can = false;
		g[x][y] = 1;
	}
