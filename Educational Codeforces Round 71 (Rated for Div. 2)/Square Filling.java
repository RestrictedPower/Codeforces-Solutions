...
/*
    Solution Created: 11:43:32 25/08/2019
...
public class Main {
	public static void solve(Reader in, Writer out) {
		...
		for(int i = 0; i<n; i++) for(int j = 0; j<m; j++) a[i][j] = in.nextInt()==1;
		ArrayList<Point> ans = new ArrayList<>();
		boolean[][] ok = new boolean[n][m];
		for(int i = 0; i<n-1; i++) {
			for(int j = 0; j<m-1; j++) {
				if(a[i][j]&&a[i][j+1]&&a[i+1][j]&&a[i+1][j+1]) {
					ans.add(new Point(i+1,j+1));
					ok[i][j] = true;
					ok[i+1][j] = true;
					ok[i][j+1] = true;
					ok[i+1][j+1] = true;
		...
		for(int i = 0; i<n; i++) {
			for(int j = 0; j<m; j++) {
				if(ok[i][j]!=a[i][j]) can = false;
			}
		}
		if(can) {
			out.println(ans.size());
			for(Point p : ans) out.println(p.x+" "+p.y);
		}else out.println(-1);
	}
