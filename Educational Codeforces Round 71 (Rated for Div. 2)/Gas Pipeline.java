...
/*
    Solution Created: 18:40:28 25/08/2019
...
public class Main {
	public static void solve(Reader in, Writer out) {
		int t = in.nextInt();
		final long safe = Long.MAX_VALUE-Integer.MAX_VALUE;
		while(t-->0) {
			long n = in.nextInt(), a = in.nextInt(), b = in.nextInt();
			char[] v = in.next().toCharArray();
			long[] dpU = new long[(int) (n+1)];
			long[] dpD = new long[(int) (n+1)];
			Arrays.fill(dpU, safe);
			Arrays.fill(dpD, safe);
			dpD[0] = b;
			for(int i = 0; i<n; i++) {
				dpU[i+1] = (v[i]=='1')?(dpU[i]+a+2*b):(Math.min(dpU[i]+a, dpD[i]+2*a)+2*b);
				dpD[i+1] = (v[i]=='1')?(safe):(Math.min(dpD[i]+a, dpU[i]+2*a)+b);
			}
			out.println(dpD[(int) n]);
		}
	}
