...
/*
    Solution Created: 15:40:33 03/11/2019
...
public class Main {
	public static void solve(Reader in, Writer out) {
		...
		while(t-->0) {
			...
			for(int i = 0; i<n; i++) {
				char c = 'x';
				if(a[i]=='R'&&s[1]>0) { wins++; s[1]--; c = 'P';}
				if(a[i]=='P'&&s[2]>0) { wins++; s[2]--; c = 'S';}
				if(a[i]=='S'&&s[0]>0) { wins++; s[0]--; c = 'R';}
				a[i] = c;
			}
			if(wins>=(n+1)/2) {
	...
	public static char reverseIdx(int a) {
		if(a == 0) return 'R';
		if(a == 1) return 'P';
		return 'S';
	}
