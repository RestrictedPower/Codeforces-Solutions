...
/*
    Solution Created: 19:49:32 17/02/2020
...
public class Main {
	...
	static ArrayList<Integer> special;
	...
	public static void solve(Reader in, Writer out) {
		...
		q.add(new Entry(n - 1, 0));

		while (!q.isEmpty()) {
			Entry cur = q.poll();
			best[cur.key] = cur.score;
			for (Integer v : con[cur.key]) {
				if (!visited[v]) {
					visited[v] = true;
					q.add(new Entry(v, cur.score + 1));
				}
			}
		}
		...
		Pair[] a = new Pair[n];
		for (int i = 0; i < n; i++) a[i] = new Pair(i, best[i]);
		Arrays.sort(a);
		int i = 0;
		while (i < n) {
			if (isSpecial[a[i].key]) {
				int j = i + 1;
				while (j < n && (!isSpecial[a[j].key])) j++;
				if (j == n) break;
				worse = Math.max(worse, best[a[j].key]+best1[a[i].key]+1);
				i = j;
			}else i++;
		}
		out.println(Math.min(worse, ans));
	}

	static class Entry {
		int key, score;

		public Entry(int key, int score) {
			this.key = key;
			this.score = score;
		}
	}
