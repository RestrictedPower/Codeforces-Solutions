...
/*
    Solution Created: 06:07:04 28/02/2020
...
public class Main {
	public static void solve(Reader in, Writer out) {
		...
		int nB = 64;
		int[] bits = new int[nB];
		for (int i = 0; i < n; i++) for (int bit = 0; bit < nB; bit++) if ((num[i] & (1 << bit)) != 0) bits[bit]++;
		int mx = 0;
		int mxIdx = 0;
		for (int i = 0; i < n; i++) {
			int gain = 0;
			for (int bit = 0; bit < nB; bit++) if ((num[i] & (1 << bit)) != 0 && bits[bit] == 1) gain += (1 << bit);
			if (gain > mx) {
				mx = gain;
				mxIdx = i;
			}
		...
		out.print(num[mxIdx] + " ");
		for (int i = 0; i < n; i++) if (i != mxIdx) out.print(num[i] + " ");
		out.println();
	}
