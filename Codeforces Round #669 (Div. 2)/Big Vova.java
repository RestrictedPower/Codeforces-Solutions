...
/*
    Solution Created: 18:05:34 08/09/2020
...
public class Main {
	public static void solve(Reader in, Writer out) {
		...
		while(t-->0) {
			...
			for(int i = 0; i<n; i++) {
				if(i==max) continue;
				vals.put(a[i], vals.getOrDefault(a[i], 0)+1);
			}
			int[] ans = new int[n];
			int prev = ans[0] = a[max];
			
			for(int i = 1; i<n; i++) {
				int bestGcd = -1;
				int val = -1;
				for (Entry<Integer, Integer> entry : vals.entrySet()) {
					if(Util.gcd(entry.getKey(), prev)>bestGcd) {
						val = entry.getKey();
						bestGcd = Util.gcd(entry.getKey(), prev);
					}
				}
				if(vals.get(val)==1) vals.remove(val);
				else vals.put(val, vals.get(val)-1);
				ans[i] = val;
				prev = bestGcd;
			}
