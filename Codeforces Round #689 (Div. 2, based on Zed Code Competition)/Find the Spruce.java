...
/*
    Solution Created: 17:16:52 11/12/2020
...
public class Main {
		...
		while(t-->0){
			...
			for(int i = 1; i<n; i++){
				...
				for(int j = 0; j<m; j++){
					...
					int maxGainFromUp = dp[i-1][j];
					int aft = suf[i][j]-1;
					int maxLR = Math.min(bef, aft)+1;
					int maxTake = Math.min(maxGainFromUp+1, maxLR);
					ans += maxTake;
					dp[i][j] = maxTake;
					bef++;
				}
	...
	public static int count(char[][] a, int x, int y){
		int c = 0;
		int curSz = 0;
		for(int i = x; i<n; i++){
			boolean allGood = true;
			for(int k = y-curSz; k<=y+curSz && allGood; k++) if(k<0 || k>=m || a[i][k]!='*') allGood = false;
			if(allGood){
				c++;
				curSz++;
			}else break;
