...
/*
    Solution Created: 06:19:13 15/12/2020
    Custom Competitive programming helper.
*/
public class Main {
	public static long ll,rr;
	public static void solve(Reader in, Writer out) {
		out.println(can(in.nextLong(), in.nextLong(), in.nextLong(), in.nextLong(), in.nextLong(), in.nextLong())?"Yes":"No");
	}
	public static boolean ok(long k){
		return ll<=k && k<=rr;
	}
	public static boolean can(long k, long l, long r, long t, long x, long y){
		//x = they drink | y = i add
		ll = l; rr = r;
		if(x==y) return ok(k-x)||ok(k+x);
		if(x>y){
			if(k+y<=r) {
				k+=y;
		...
		boolean[] vis = new boolean[(int) x];
		while(true){
			long rem = (k-l)/x;
			int pos = (int) ((k-l)%x);
			k-=rem*x;
			t-=rem;
			if(t<=0||vis[pos]) return true;
			vis[pos] = true;
			k+=y;
			if(!ok(k)) return false;
		}
