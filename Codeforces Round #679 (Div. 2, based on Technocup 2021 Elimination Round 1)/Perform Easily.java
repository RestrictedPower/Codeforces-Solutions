...
/*
    Solution Created: 17:48:36 25/10/2020
...
public class Main {
	public static void solve(Reader in, Writer out) {
		...
		for(int i = 0; i<n; i++) for(int j = 0; j<6; j++) all.add(new Pair(b[i]-a[j], i));
...
//		Util.dbg(all);
		while(true) {
			if(count>=n) {
				ans = Math.min(ans, all.get(r-1).val-all.get(l).val);
				if(--f[all.get(l).id]==0) count--;
				l++;
			}else {
				if(f[all.get(r).id]++==0) count++;
				r++;
				if(r>=all.size()) break;
			}
		}
