...
/*
    Solution Created: 15:48:10 25/10/2020
...
public class Main {
	public static void solve(Reader in, Writer out) {
		...
		TreeSet<Integer> freePos = new TreeSet<Integer>();
		for(int i = 0; i<n; i++) freePos.add(i);
		boolean can = true;
		int[] ans = new int[n];
		int idx = -1;
		int size = 0;
		for(int i = 0; i<2*n; i++) {
			int v = q[i];
			if(v<0) {
				Integer next = freePos.higher(idx);
				if(next == null) next = freePos.first();
				idx = next;
				size++;
			}
			else {
				...
				if(ans[idx]!=0) {
					...
					Integer next = freePos.lower(idx);
					if(next == null) next = freePos.last();
					idx = next;
				}
				ans[idx] = v;
				freePos.remove(idx);
