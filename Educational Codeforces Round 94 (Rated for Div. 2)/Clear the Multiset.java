...
/*
    Solution Created: 15:15:18 29/08/2020
...
public class Main {
	...
	public static int solve(int l, int r) {
		if(l>r) return 0;
		int min = l;
		for(int i = l; i<=r; i++) if(a[min]>a[i]) min = i;
		ArrayList<Integer> idxs = new ArrayList<Integer>();
		for(int i = l; i<=r; i++) if(a[i]==a[min]) idxs.add(i);
		int minV = a[min];
		int count = minV;
		for(int i = l; i<=r; i++) a[i]-=minV;
		count += solve(l, idxs.get(0)-1) + solve(idxs.get(idxs.size()-1)+1, r);
		for(int i = 0; i<idxs.size()-1; i++) count += solve(idxs.get(i)+1, idxs.get(i+1)-1);
