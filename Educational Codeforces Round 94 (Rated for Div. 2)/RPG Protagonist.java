...
/*
    Solution Created: 11:05:14 29/08/2020
...
public class Main {
	public static void solve(Reader in, Writer out) {
		...
		while(t-->0) {
			...
			int swords = in.nextInt(), axes = in.nextInt();
			int swordWeight = in.nextInt(), axeWeight = in.nextInt();
			if(swordWeight>axeWeight) {
				int tmp = swords;
				swords = axes;
				axes = tmp;
				tmp = swordWeight;
				swordWeight = axeWeight;
				axeWeight = tmp;
			}
			int ans = 0;
			int totalSwords = Math.min(swords, p/swordWeight + f/swordWeight);
			for(int firstSwords = 0; firstSwords<=totalSwords; firstSwords++) {
				int secondSwords = totalSwords-firstSwords;
				if(firstSwords*swordWeight>p || secondSwords*swordWeight>f) continue;
				int pLeft = p-firstSwords*swordWeight, fLeft = f-secondSwords*swordWeight;
				int cur = (firstSwords+secondSwords) + Math.min(axes, pLeft/axeWeight + fLeft/axeWeight);
