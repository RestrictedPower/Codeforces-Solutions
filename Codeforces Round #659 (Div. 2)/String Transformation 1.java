...
/*
    Solution Created: 23:10:55 24/07/2020
...
public class Main {
	public static void solve(Reader in, Writer out) {
		...
		while(t-->0) {
			...
			boolean can = true;
			for(int i = 0; i<n; i++) if(a[i]>b[i]) can = false;
			if(!can) {
				out.println(-1);
				continue;
			}
			int ans = 0;
			boolean[][] tr = new boolean[26][26];
			for(int i = 0; i<n; i++) tr[a[i]-'a'][b[i]-'a'] = true;
			for(int i = 0; i<26; i++) {
				int count = 0;
				int min = Integer.MAX_VALUE;
				for(int j = i+1; j<26; j++) if(tr[i][j]) {
						count++;
						min = Math.min(j, min);
				}
				if(count == 0) continue;
				ans++;
				for(int j = 0; j<26; j++) if(tr[i][j]) tr[min][j] = true;
			}
			out.println(ans);
		}
	}
