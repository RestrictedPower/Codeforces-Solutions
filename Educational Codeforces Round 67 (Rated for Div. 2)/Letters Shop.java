...
/*
    Solution Created: 18:23:13 30/06/2019
...
public class Main {
	public static void solve(Reader in, Writer out) {
		...
		while(t-->0) {
			...
			int[] need = new int[26];
			for(int i = 0; i<b.length; i++) need[b[i]-'a']++;
			int ans = 0;
			for(int j = 0; j<26; j++)ans = Math.max(ans,Util.lowerBound(f[j],need[j])+1);
			out.println(ans);
		}
	}
