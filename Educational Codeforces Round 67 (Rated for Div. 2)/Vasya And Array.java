...
/*
    Solution Created: 22:05:20 30/06/2019
...
public class Main {
		public static void solve(Reader in, Writer out) {
			...
			for(int i = 0; i<m; i++) {
				...
				int r = in.nextInt()-1;
				if(x==1) {
					for(int j = l; j<r; j++) state[j] = false;
				}
				else ar.add(new Pair(l,r));
			}
			Collections.sort(ar);
			for(int i = 0; i<n; i++) ans[i] = 100000;
			boolean can = true;
			for(int i = 0; i<ar.size()&&can; i++) {
				int l = ar.get(i).l, r = ar.get(i).r;
				can = false;
				for(int j = l; j<r; j++) {
					if(state[j]) {
						can = true;
						for(int k = j+1; k<n; k++) ans[k]--;
					}
				}
			}
			if(can) {
