...
/*
    Solution Created: 04:54:33 20/05/2020
    Custom Competitive programming helper.
*/

public class Main {
	static int[] tree;
	public static void add(int idx, int l, int r, int v) {
		if(l==r) tree[idx]++;
		else {
			int md = (l+r)/2;
			if(v<=md) add(2*idx+1, l, md, v);
			else add(2*idx+2, md+1, r, v);
			tree[idx] = tree[2*idx+1]+tree[2*idx+2];
		}
	}
	public static int query(int idx, int min, int max, int l, int r) {
		if(min>r || max<l) return 0;
		if(min<=l && l<=max) return tree[idx];
		int md = (l+r)/2;
		return query(2*idx+1, min, max, l, md) + query(2*idx+2, min, max, md+1, r);
	}
	public static void solve(Reader in, Writer out) {
		...
		tree = new int[4*n];
		add(0, 0, n-1, map.get(a[0]));
		long ans = 0;
		for(int i = 1; i<n-1; i++) {
			int val = map.get(a[i]);
			long largerBefore = query(0, val, n-1, 0, n-1);
			long smallerBefore = i-largerBefore;
			long smallerTotal = Arrays.binarySearch(b, a[i]);
			long smallerNext = smallerTotal-smallerBefore;
			ans += smallerNext*largerBefore;
			add(0, 0, n-1, val);
		}
		out.println(ans);
	}
