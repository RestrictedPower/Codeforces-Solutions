...
/*
    Solution Created: 20:56:00 09/06/2019
...
public class Main {
	public static void main(String[] args) {
		...
		for(int i = 0; i<n; i++) {
			for(int j = 0; j<6; j++) {
				if(a[i]!=nums[j]) continue;
				cnt[j] = j==0?cnt[j]+1:Math.min(cnt[j]+1, cnt[j-1]);
			}
		}
		int min = Integer.MAX_VALUE;
		for(int i = 0; i<6; i++) min = Math.min(min, cnt[i]);
		System.out.println(a.length-6*min);
	}
