...
/*
    Solution Created: 17:47:27 14/03/2020
...
public class Main {
	public static void solve(Reader in, Writer out) {	
		...
		int leaves = 0, other = n-2;
		for(int i = 0; i<n-1; i++) {
			if((deg[a[i][0]]==1)||(deg[a[i][1]]==1)) out.println(leaves++);
...
static class Util{
		...
		//##################################################################################################################
		//# Shuffling an array before sorting makes the java quickSort O(nlogn) instead of O(n^2) for anti-java testcases. #
		//##################################################################################################################
