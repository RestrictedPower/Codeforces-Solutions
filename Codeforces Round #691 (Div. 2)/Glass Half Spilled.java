...
/*
    Solution Created: 06:24:53 20/12/2020
...
public class Main {
	public static void solve(Reader in, Writer out) {
		...
		int[][] dp = new int[n+1][10005]; //dp[i][j] -> max water by taking i glasses with the total of j capacity.
		for(int i = 0; i<n+1; i++) for(int j = 0; j<10005; j++) dp[i][j] = -1;
		dp[0][0] = 0;
		for(int i = 0; i<n; i++) {
			for(int use = n-1; use>=0; use--) {
				for(int am = 10000; am-a[i]>=0; am--) {
					if(dp[use][am-a[i]]==-1) continue;
					dp[use+1][am] = Math.max(dp[use+1][am], dp[use][am-a[i]]+b[i]);
				}
			}
		}
		for(int k = 1; k<=n; k++) {
			...
			for(int i = 1; i<=10000; i++) {
				...
				ans = Math.max(ans, Math.min(i, dp[k][i]+(sum-dp[k][i])*0.5));
			}
			out.print(ans+" ");
		}
		out.println();
