...
/*
    Solution Created: 12:21:21 19/12/2020
    Custom Competitive programming helper.
*/
public class Main {
	public static void solve(Reader in, Writer out) {
		int n = in.nextInt(), m = in.nextInt();
		long[] a = in.nl(n), b  = in.nl(m);
		Util.sortArray(a);
		long gcd = 0;
		for(int i = 1; i<n; i++) gcd = Util.gcd(gcd, a[i]-a[i-1]);
		for(int i = 0; i<m; i++) out.print(Util.gcd((b[i]+a[0]), gcd)+" ");
		out.println();
	}
