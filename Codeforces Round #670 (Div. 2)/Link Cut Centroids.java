...
/*
    Solution Created: 17:50:50 12/09/2020
...
public class Main {
	...
	static int n;
	static int[] sz;
	static ArrayList<Integer> centr;
	public static void solve(Reader in, Writer out) {
		int t = in.nextInt();
		while(t-->0) {
			n = in.nextInt();
			sz = new int[n];
			centr = new ArrayList<Integer>();
			adj = new ArrayList[n];
			...
			dfs(0, -1);
			if(centr.size()==1) {
				out.println("1 "+(adj[0].get(0)+1));
				out.println("1 "+(adj[0].get(0)+1));
			}else {
				int c1 = centr.get(0), c2 = centr.get(1);
				int e1 = findEdge(c1, c2);
				out.println((e1+1)+" "+(adj[e1].get(0)+1));
				out.println((e1+1)+" "+(c2+1));
			}
		}
	}
	static int findEdge(int i, int prev) {
		if(adj[i].size()==1) return i;
		for(Integer nxt : adj[i]) {
			if(nxt==prev) continue;
			return findEdge(nxt, i);
		}
		return -1;
	}
	static void dfs(int i , int prev) {
        ...
        for (Integer v : adj[i]) if (v != prev) {
                dfs(v, i);
                sz[i] += sz[v];
                if (sz[v] > n / 2) centroid = false;
        }
        if (n - sz[i] > n / 2) centroid = false;
        if (centroid) centr.add(i);
	}
