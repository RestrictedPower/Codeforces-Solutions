...
/*
    Solution Created: 15:52:17 03/05/2020
...
public class Main {
	public static void solve(Reader in, Writer out) {
		...
		while (t-- > 0) {
			...
			for (int i = 0; i < n; i++) toUse.add(a[i]);
			if (toUse.size() > k) out.println(-1);
			else {
				ArrayList<Integer> use = new ArrayList<Integer>(toUse);
				while(use.size()<k) use.add(1);
				out.println(use.size()*n);
				for(int i = 0; i<n; i++) for(Integer j : use) out.print(j+" ");
				out.println();
			}
		}
	}
