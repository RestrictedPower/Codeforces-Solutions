...
/*
    Solution Created: 18:00:14 19/03/2020
...
public class Main {
	public static void solve(Reader in, Writer out) {
		...
		while (t-- > 0) {
			...
			ArrayList<Character> xr = new ArrayList<Character>();
			xr.addAll(x);
			if (l < h) {
				a = Arrays.copyOfRange(a, l, h+1);
				n = a.length;
				ArrayList<Character> c = findX(a);
				Util.reverse(a);
				ArrayList<Character> d = findX(a);
				if(c.size()>d.size())  x.addAll(c);
				else x.addAll(d);
			}else {
				if(a.length%2==1) xr.remove(xr.size()-1);
			}
			for(Character c : x) out.print(c);
			Collections.reverse(xr);
			for(Character c : xr) out.print(c);
			out.println();
		}
	}
	public static ArrayList<Character> findX(char[] a) {
		ArrayList<Character> ret = new ArrayList<Character>();
		ArrayList<Character> rev = new ArrayList<Character>();
		char[] x = new char[a.length*2+1];
		int n = x.length;
		int idx = 0;
		for(int i = 0; i<a.length; i++) x[idx++] = a[i];
		x[idx++] = '#';
		idx = n-1;
		for(int i = 0; i<a.length; i++) x[idx--] = a[i];
        ...
        for (int i = 1; i < n; ++i) {
            ...
            while (prev_idx > 0 && x[i] != x[prev_idx])
            {
                prev_idx = lps[prev_idx - 1];
            }

            lps[i] = prev_idx + (x[i] == x[prev_idx] ? 1 : 0);
        }
        int ls = lps[n - 1];
        for(int i = 0; i<ls/2; i++) ret.add(a[i]);
        if(ls%2==1) ret.add(a[ls/2]);
        for(int i = 0; i<ls/2; i++) rev.add(a[i]);
