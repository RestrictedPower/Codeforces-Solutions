...
/*
    Solution Created: 12:48:44 07/09/2019
	...
	public static void solve(Reader in, Writer out) {
		...
		while(t-->0) {
			...
			for(int i = 0; i<n; i++) {
				...
				if(a>maxDamage) maxDamage = a;
			}
			int ans = 0;
			if(mostWorth<=0) {
				if(maxDamage>=x) ans = 1;
				else ans = -1;
			}else {
				int need = (int) Math.max(0,Math.ceil((x-maxDamage)/(double)mostWorth));
				int dmg = mostWorth*need;
				ans+=need;
				if(dmg<x) ans++;
			}
			out.println(ans);
