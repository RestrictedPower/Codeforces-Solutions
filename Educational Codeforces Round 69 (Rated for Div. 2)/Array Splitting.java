...
/*
    Solution Created: 18:39:17 22/07/2019
...
public class Main {
	public static void solve(Reader in, Writer out) {
		...
		Pair[] dif = new Pair[n-1];
		for(int i = 0; i<n-1; i++) dif[i] = new Pair(a[i+1]-a[i] , i);
		Arrays.sort(dif);
		boolean[] br = new boolean[n];
		for(int i = 0; i<k-1; i++) br[dif[i].idx]=true;
		long ans = 0;
		int lst = 0;
		for(int i = 0; i<n; i++) {
			if(br[i]) {
