...
/*
    Solution Created: 13:02:19 18/10/2020
...
public class Main {
	...
	public static void solve(Reader in, Writer out) {
		...
		while(t-->0) {
			n = in.nextInt();
			g = new char[n][n];
			for(int i = 0; i<n; i++)  g[i] = in.nca();
			char[][] gC  = new char[n][n];
			for(int i = 0; i<n; i++) for(int j = 0; j<n; j++) gC[i][j] = g[i][j];
			int[] dx = {0,1,n-1,n-2};
			int[] dy = {1,0,n-2,n-1};
			for(int msk = 0; msk<(1<<4); msk++) {
				...
				for(int i = 0; i<4; i++) {
					if(isOn(msk, i)) {
						g[dx[i]][dy[i]] = sp(gC[dx[i]][dy[i]]);
						totalOn++;
					} else {
						g[dx[i]][dy[i]] = gC[dx[i]][dy[i]];
					}
				}
				if(totalOn>2) continue;
				boolean c1 = canReach('1');
				boolean c2 = canReach('0');
				if(!c1 && !c2) {
					ArrayList<Integer> ans = new ArrayList<Integer>();
					for(int i = 0; i<n; i++) for(int j = 0; j<n; j++) if(g[i][j]!=gC[i][j]) {
						ans.add(i+1);
						ans.add(j+1);
					}
			...
			throw new RuntimeErrorException(null,"dab");
		}
	}
	public static char sp(char c) {
		if(c=='1') return  '0';
		return '1';
	}
	public static boolean isOn(int v, int b) {
		return ((v&1<<b) != 0);
	}
	public static boolean canReach(char v) {
		can = false;
		vis = new boolean[n][n];
		dfs(0, 0, v);
		return can;
	}
	public static void dfs(int i, int j, char v) {
