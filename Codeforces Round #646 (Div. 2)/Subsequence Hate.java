...
/*
    Solution Created: 17:56:27 31/05/2020
...
public class Main {
	public static void solve(Reader in, Writer out) {
		...
		while(t-->0) {
			...
			int n = a.length;
			int[] pre = new int[n+1];
			for(int i = 0; i<n; i++)pre[i+1] = pre[i] + (a[i]=='0'?0:1);
			int ans = Math.min(pre[n], n-pre[n]);
			for(int i = 1; i<=n; i++) {
				int bef = pre[i], af = pre[n]-pre[i];
				int pAm = i, nAm = n-i;
				ans = Math.min(ans, bef+(nAm-af));
				ans = Math.min(ans, (pAm-bef)+af);
			}
			out.println(ans);
		}
	}
