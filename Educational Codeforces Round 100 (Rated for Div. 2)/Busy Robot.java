...
/*
    Solution Created: 17:45:24 17/12/2020
...
public class Main {
	public static void solve(Reader in, Writer out) {
		...
		while(T-->0){
			...
			int curPos = 0, goingTo = 0;
			for(int i = 0; i<n-1; i++){
				int timeIHave = c[i+1][0]-c[i][0];
				if(curPos!=goingTo){
					int nextPos = curPos;
					if(Math.abs(curPos-goingTo)<=timeIHave){
						nextPos = goingTo;
					}else{
						if(curPos>goingTo) nextPos-=timeIHave;
						else nextPos+=timeIHave;
					}
					if(isBet(curPos, nextPos, c[i][1])) ans++;
					curPos = nextPos;
				}else{
					goingTo = c[i][1];
					int nextPos = curPos;
			...
			if(curPos==goingTo|| isBet(curPos, goingTo, c[n-1][1])) ans++;
			out.println(ans);
		}
	}
	public static boolean isBet(int l, int r, int pos){
		if(l>r){
			int tmp = l;
			l = r;
			r = tmp;
