...
/*
    Solution Created: 17:01:12 17/12/2020
...
public class Main {
	public static void solve(Reader in, Writer out) {
		...
		while(t-->0){
			int n = in.nextInt();
			long[] a = in.nl(n);
			for(long i : a) out.print(closestPowerOf2(i)+" ");
			out.println();
		}
	}
	public static long closestPowerOf2(long v){
		long pow = 1;
		while(true){
			if(pow==v) return pow;
			if(pow>v){
				long dprev = (long)Math.abs(pow/2-v), dnext = (long)Math.abs(pow-v);
				if(dprev<dnext || pow>1000000000) return pow/2;
				return pow;
			}
			pow*=2;
		}
