...
/*
    Solution Created: 20:43:30 26/04/2020
	...
	public static void solve(Reader in, Writer out) {
		...
		while(t-->0) {
			...
			while(q-->0) {
				...
				long d = r-l+1;
				long lq = countHits(l-1, a, b), rq = countHits(r, a, b);
				out.print((d-(rq-lq))+" ");
			}
			out.println();
		}
	}
	public static long countHits(long idx, int a, int b) {
		int d = a*b;
		long ans = Math.max(0, (idx/d))*f[d];
		ans+= f[(int) (idx%d)];
		return ans;
	}
