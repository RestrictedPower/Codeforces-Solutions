...
/*
    Solution Created: 19:16:59 19/09/2019
...
public class Main {
	public static void solve(Reader in, Writer out) {
		...
		while(q-->0) {
			...
			Queue<Deal> deals = new LinkedList<Deal>();
			deals.add(new Deal(a[0], 0));
			deals.add(new Deal(a[0]+1, p[0]));
			deals.add(new Deal(a[0]+2, 2*p[0]));
			for(int i = 1; i<n; i++) {
				ArrayList<Deal> ds = new ArrayList<>();
				while(!deals.isEmpty()) ds.add(deals.poll());
				Collections.sort(ds);
				boolean[] used = new boolean[3];
				for(Deal d : ds) {
					if(d.height==a[i]) {
						if(!used[1]) deals.add(new Deal(a[i]+1, d.price+p[i]));
						if(!used[2]) deals.add(new Deal(a[i]+2, d.price+2*p[i]));
						used[1] = true;
						used[2] = true;
					}else if(d.height==a[i]+1) {
						if(!used[0]) deals.add(new Deal(a[i], d.price));
						if(!used[2]) deals.add(new Deal(a[i]+2, d.price+2*p[i]));
						used[0] = true;
						used[2] = true;
					}else if(d.height==a[i]+2) {
						if(!used[0]) deals.add(new Deal(a[i], d.price));
						if(!used[1]) deals.add(new Deal(a[i]+1, d.price+p[i]));
						used[0] = true;
						used[1] = true;
			...
			while(!deals.isEmpty())best = Math.min(best, deals.poll().price);
			out.println(best);
		}
	}
	static class Deal implements Comparable<Deal>{
		long height, price;
		public Deal(long height, long price) {
			this.height = height;
			this.price = price;
		}
		@Override
