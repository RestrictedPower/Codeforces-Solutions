...
/*
    Solution Created: 02:02:07 12/06/2019
...
public class Main {
	...
	public static void main(String[] args) {
		...
		for(int i = 0; i<n&&!isCross; i++) {
			for(int j = 0; j<m&&!isCross; j++) {
				if(is(i,j)) {
					if(!is(i+1, j)||!is(i-1, j)||!is(i, j+1)||!is(i, j-1)) continue;
					isCross = true;
					b[i][j] = false;
					for(int k = i+1; is(k, j); k++) b[k][j] = false;
					for(int k = i-1; is(k, j); k--) b[k][j] = false;
					for(int k = j+1; is(i, k); k++) b[i][k] = false;
					for(int k = j-1; is(i, k); k--) b[i][k] = false;
				}
			}
		}
		boolean left = false;
		for(int i = 0; i<n; i++) for(int j = 0; j<m; j++) if(b[i][j]) left = true;
		System.out.println((isCross&&!left)?"YES":"NO");
	}
