...
/*
    Solution Created: 19:57:50 13/04/2020
...
public class Main {
	public static void solve(Reader in, Writer out) {
		...
		while(t-->0) {
			...
			for(int i = 0; i<9; i++) {
				String r = in.next();
				for(int j = 0; j<9; j++) b[i][j] = (r.charAt(j)-'0');
			}
			int s = 0;
			int k = 0;
			for(int i = 0; i<3; i++) {
	...
	public static int find(int i, int j, int[][] b) {
		for(int k = 0; k<3; k++) {
			for(int l = 0; l<3; l++) {
				if(b[k+i][l+j]==1) {
					return k;
	...
	public static int choose(int i, boolean[] used) {
		for(int j = 0; j<3; j++) if((!used[j])&&(j!=i)) {
			used[j] = true;
			return j;
		}
		return -1;
