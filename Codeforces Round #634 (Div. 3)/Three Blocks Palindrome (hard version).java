...
/*
    Solution Created: 12:19:14 15/04/2020
	...
	public static void solve(Reader in, Writer out) {
		...
		while(t-->0) {
			...
			for(int i = 1; i<=n; i++) {
				...
				int g = get(0, i-1, a[i-1]);
				if(!L.containsKey(g)) L.put(g, new HashMap<Integer, Integer>());
				L.get(g).put(a[i-1], i-1);
			}
			for(int j = 1; j<MXA; j++) {
				for(int use = 1; use<n; use++) {
					int tot = get(0, n-1, j);
					if(use>tot/2) break;
					int li = L.get(use).get(j);
					int ri = L.get(tot-use+1).get(j);
					for(int mid = 1; mid<MXA; mid++) 
						ans = Math.max(ans, 2*use+get(li+1, ri-1, mid));
				}
			}
			out.println(ans);
		}
