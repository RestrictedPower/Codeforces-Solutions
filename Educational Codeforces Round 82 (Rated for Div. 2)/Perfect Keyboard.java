...
/*
    Solution Created: 18:19:32 12/02/2020
	...
	public static void solve(Reader in, Writer out) {
		...
		while (t-- > 0) {
			...
			for(int i = 1; i<n&&can; i++) {
				char cur = a[i];
				if(ans[idx+1]==cur) idx++;
				else if(ans[idx-1]==cur) idx--;
				else if(ans[idx+1]=='*') {
					if(used.contains(cur)) can = false;
					used.add(cur);
					ans[++idx] = cur;
				}
				else if(ans[idx-1]=='*') {
			...
			for(int i = 0; i<26; i++) if(!used.contains((char)(i+'a'))) left.add((char) (i+'a'));
			if(can) {
				out.println("YES");
				int j = h/2;
				while(ans[j]!='*')j++;
				for(int i = 0; i<left.size(); i++) ans[i+j] = left.get(i);
				for(int i = 0; i<h; i++) if(ans[i]!='*')out.print(ans[i]);
				out.println();
			}else {
				out.println("NO");
			}
