...
/*
    Solution Created: 19:16:39 26/06/2019
...
static class Solution {
	public static void solve(Reader in, Writer out) {
		...
		while(q-->0) {
			...
			for(int i = 0; i<n; i++) {
				int a = in.nextInt();
				int b = in.nextInt()==1?0:1;
				f[a]++;
				WE[a]+=b;
			}
			PriorityQueue<Pair> cont = new PriorityQueue<>(Collections.reverseOrder());
			for(int i = 0; i<n+1; i++) {
				if(f[i]>0) {
					cont.add(new Pair(i, f[i], WE[i]));
				}
			}
			int ans1 = 0;
			int ans2 = 0;
			while(!cont.isEmpty()) {
				Pair p = cont.poll();
				//System.out.println(p.id+" "+p.val+" "+p.wannaEeat);
				ans1+=p.val;
				ans2+=p.val-p.wannaEeat;
				if(p.val<=1) break;
				while((!cont.isEmpty())&&cont.peek().val==p.val) {
					Pair pop = cont.poll();
					cont.add(new Pair(pop.id, pop.val-1, Math.max(0, pop.wannaEeat-1)));
				}
			}
	...
	static class Pair implements Comparable<Pair>{
		...
		int val;
		int wannaEeat;
		public Pair(int id, int val, int wannaEat) {
			this.id = id;
			this.val = val;
			this.wannaEeat = wannaEat;
		}
		@Override
		public int compareTo(Pair o) {
			if(this.val == o.val) {
				return o.wannaEeat - this.wannaEeat;
			}
