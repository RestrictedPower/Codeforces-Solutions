...
/*
    Solution Created: 18:52:51 12/08/2020
...
public class Main {
	...
	static ArrayList<Pair> ans = new ArrayList<Main.Pair>();
	static int cnt=0, n,m;
	public static void solve(Reader in, Writer out) {
		n = in.nextInt(); m = in.nextInt();
		int sx = in.nextInt(), sy = in.nextInt();
		visited = new boolean[n][m];
		ans.add(new Pair(sx,sy));
		for(int j = m-1; j>=0; j--) if(j+1!=sy) ans.add(new Pair(sx,j+1));
		for(int j = 0; j<m; j++) {
			if(j%2==0) {
				for(int i = 0; i<n; i++) {
					if(i+1!=sx) ans.add(new Pair(i+1, j+1));
				}	
			}else {
				for(int i = n-1; i>=0; i--) {
					if(i+1!=sx) ans.add(new Pair(i+1, j+1));
		...
		for(Pair p : ans) out.println(p.x +" "+p.y);
	}
	static class Pair{
		int x, y;
