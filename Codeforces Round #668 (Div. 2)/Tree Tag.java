...
/*
    Solution Created: 20:41:56 06/09/2020
    Custom Competitive programming helper.
*/

public class Main {
	static int t, n, a, b, da, db;
	...
	public static void solve(Reader in, Writer out) {
		...
		while (t-- > 0) {
			init(in);
			if ((db < 2 * da + 1) || canReach(a, b, -1, 0) || (diameter() < 2 * da + 1))
				out.println("Alice");
			else
				out.println("Bob");
		}
	...
	public static boolean canReach(int i, int bob, int prev, int moves) {
		if (moves > da)
			return false;
		if (i == b)
			return true;
		...
		for (Integer next : adj[i]) {
			if (next == prev)
				continue;
			can |= canReach(next, bob, i, moves + 1);
		}
		return can;
	}
	...
	public static void init(Reader in) {
		n = in.nextInt();
		a = in.nextInt() - 1;
		b = in.nextInt() - 1;
		da = in.nextInt();
		...
		for (int i = 0; i < n - 1; i++) {
			int v = in.nextInt() - 1, b = in.nextInt() - 1;
			adj[v].add(b);
			adj[b].add(v);
		}
	}
