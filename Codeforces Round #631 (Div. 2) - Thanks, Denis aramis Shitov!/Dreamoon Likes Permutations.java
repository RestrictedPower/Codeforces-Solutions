...
/*
    Solution Created: 18:38:44 03/04/2020
		...
		while(t-->0) {
			...
			boolean[] can1 = new boolean[n];
			{
				...
				for(int i = 0; i<n; i++) {
					...
					if(dif==i+1&&max==i+1) can1[i] = true;
				}
			}
			Util.reverse(a);
			boolean[] can2 = new boolean[n];
			{
				...
				for(int i = 0; i<n; i++) {
					...
					dif++;
					max = Math.max(a[i], max);
					if(dif==i+1&&max==i+1) can2[i] = true;
				}
			}
			Util.reverse(can2);
			ArrayList<String> ans = new ArrayList<String>();
			if(can2[0]) ans.add("0 "+(n-1));
			for(int i = 0; i<n-1; i++) {
				if(can1[i]&&can2[i+1])ans.add((i+1)+" "+(n-i-1));
			}
			if(can1[n-1]) ans.add((n-1)+" 0");
