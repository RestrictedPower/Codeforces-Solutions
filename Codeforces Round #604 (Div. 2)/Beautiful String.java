...
/*
    Solution Created: 17:59:04 08/02/2020
...
public class Main {
	public static void solve(Reader in, Writer out) {
		...
		while(t-->0) {
			...
			int n = a.length;
			if(n==1) a[0] = a[0]=='?'?'a':a[0]; 
			else for(int i = 0; i<n; i++) {
				if(a[i]!='?') continue;
				int nx = (i+1==n?a[i-1]:a[i+1]);
				int pr = (i-1==-1?a[i+1]:a[i-1]);
				for(int j = 0; j<3; j++) {
					a[i] = (char) ('a'+j);
					if(a[i]!=nx&&a[i]!=pr) break;
				}
			}
			boolean ok = true;
			for(int i = 0; i<n-1; i++) if(a[i]==a[i+1]) ok = false;
			if(ok) for(int i = 0; i<n; i++) out.print(a[i]);
			else out.print(-1);
			out.println();
		}
	}
