...
/*
    Solution Created: 19:39:00 23/06/2020
...
public class Main {
	public static void solve(Reader in, Writer out) {
		...
		while(t-->0) {
			...
			char[] a = in.nca();
			int l,r;
			for(l = 0; l<n; l++) if(a[l]=='1') break;
			for(r = n-1; r>=0; r--) if(a[r]=='0') break;
			
			if(r<l) for(Character c : a) out.print(c); 
			else {
				for(int i = 0; i<l; i++) out.print(a[i]);
				for(int i = r; i<n; i++) out.print(a[i]);
			}
			out.println();
		}
	}
