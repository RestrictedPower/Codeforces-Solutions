...
/*
    Solution Created: 03:13:08 21/07/2020
...
public class Main {
	public static void solve(Reader in, Writer out) {
		...
		while (t-- > 0) {
			...
			Pair[] a = new Pair[m];
			for(int i = 0; i<m; i++) a[i] = new Pair(in.nextLong(), in.nextLong());
			Util.sortArray(a);
			long[] pre = new long[m];
			pre[0] = a[0].a;
			for(int i = 1; i<m; i++) pre[i] = pre[i-1]+a[i].a;
			long ans = 0;
			for(int i = 0; i<m; i++) {
				int l = 0, r = Math.min(m-1,n-1), take = -1;
				while(l<=r) {
					int md = (l+r)/2;
					if(a[md].a>a[i].b) {
						take = md;
				...
				int left = n-take-1;
				long cur = take==-1?0:pre[take];
				if(i>take && left>0) cur+= a[i].a + a[i].b*(left-1);
				else cur+=  a[i].b*left;
