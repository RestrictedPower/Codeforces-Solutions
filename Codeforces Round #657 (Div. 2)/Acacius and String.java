...
/*
    Solution Created: 02:00:05 20/07/2020
...
public class Main {
	public static void solve(Reader in, Writer out) {
		...
		while(t-->0) {
			int n = in.nextInt();
			char[] a = in.nca();
			char[] b = "abacaba".toCharArray();
			int foundTimes = findTimes(a, b);
			if(foundTimes>1) out.println("NO");
			else if(foundTimes==1) {
				out.println("YES");
				for(char c : a) out.print(c=='?'?'z':c);
				out.println();
			}else {
				boolean ok = false;
				for(int i = 0; i<n; i++) {
					...
					if(can) {
						for(int j = 0; j<b.length; j++) A[i+j] = b[j];
						if(findTimes(A, b)==1) {
							out.println("YES");
							for(char c : A) out.print(c=='?'?'z':c);
							out.println();
	...
	public static int findTimes(char[]a, char[] b) {
		int foundTimes = 0;
		int n = a.length;
		for(int i = 0; i<n; i++) {
			int idx = 0;
