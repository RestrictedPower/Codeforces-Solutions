...
/*
    Solution Created: 15:26:40 04/12/2020
...
public class Main {
	public static void solve(Reader in, Writer out) {
		...
		while(t-->0){
			...
			for(int i = 1; i<n-1; i++){
				long prev = Math.abs(a[i]-a[i-1]), next = Math.abs(a[i]-a[i+1]);
				long extra = Math.abs(a[i-1]-a[i+1]);
				ans = Math.min(ans, d-prev-next+extra);
			}
			ans = Math.min(ans, d-Math.abs(a[0]-a[1]));
			ans = Math.min(ans, d-Math.abs(a[n-1]-a[n-2]));
			out.println(ans);
		}
	}
