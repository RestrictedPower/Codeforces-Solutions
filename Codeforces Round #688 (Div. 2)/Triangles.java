...
/*
    Solution Created: 16:07:01 04/12/2020
    Custom Competitive programming helper.
*/

public class Main {
	static int[] best = new int[10];
	static char[][] ar;
	static int n;
	public static void solve(Reader in, Writer out) {
		...
		while(t-->0){
			...
			for(int i = 0; i<10; i++) out.print(best[i]+" ");
			out.println();
		}
	}
	static int check(char[][] a){
		int[] lastPos = new int[10];
		int[][] lastPosRow = new int[n][10];
		for(int i = n-1; i>=0; i--){
			for(int j = 0; j<n; j++){
				lastPos[a[i][j]-'0'] = Math.max(lastPos[a[i][j]-'0'], i);
				lastPosRow[i][a[i][j]-'0'] = Math.max(lastPosRow[i][a[i][j]-'0'], j);
			}
		}
		...
		for(int i = 0; i<n; i++){
			for(int j = 0; j<n; j++){
				int c = a[i][j]-'0';
				//case for same in row
				{
					int idx = lastPosRow[i][c];
					
					int b = idx-j;
					int h = n-1-i;
					best[c] = Math.max(best[c], b*h);
				}
				
				//case for not same in row
				{
					int b = n-1-j;
					int h = lastPos[c]-i;
					best[c] = Math.max(best[c], b*h);
	...
	static char[][] mirror(int n, char[][] a){
		char[][] b = new char[n][n];
		for(int i = 0; i<n; i++){
			for(int j = 0; j<n; j++){
				b[i][j] = a[i][j];
	...
	static void rotateMatrix(int N, char mat[][])  { 
	        for (int x = 0; x < N / 2; x++) { 
