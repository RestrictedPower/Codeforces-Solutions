...
/*
    Solution Created: 18:05:30 04/07/2020
	...
	public static void solve(Reader in, Writer out) {
		...
		while(t-->0) {
			...
			for(int i = 0; i<n; i++) for(int j = 0; j<m; j++) {
				a[i][j] = in.nextInt();
				if(a[i][j]>4) can = false;
				if(((i==0&&j==0) || (i==0&&j==m-1)|| (i==n-1&&j==0) || (i==n-1 && j == m-1)) && a[i][j]>2) can = false;
				if((i==0 || j==0 || i==n-1 || j==m-1) && a[i][j]>3) can = false;
			}
			if(!can) {
				out.println("NO");
				continue;
			...
			for(int i = 0; i<n; i++) {
				for(int j = 0; j<m; j++) {
					if((i==0&&j==0 ) || (i==0&&j==m-1)|| (i==n-1&&j==0) || (i==n-1 && j == m-1)) out.print(2+" ");
					else if(i==0 || j==0 || i==n-1 || j==m-1) out.print(3+" ");
					else out.print(4+" ");
				}
				out.println();
			}
