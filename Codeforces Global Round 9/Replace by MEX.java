...
/*
    Solution Created: 13:26:13 05/07/2020
...
public class Main {
	...
	public static void solve(Reader in, Writer out) {
		...
		while(t-->0) {
			...
			HashSet<Integer> incomplete = new HashSet<Integer>();
			for(int i = 0; i<n; i++) if(a[i]!=i) incomplete.add(i);
			while(incomplete.size()>0) {
				...
				if(mex==n) {
					...
					incomplete.remove(mex);
					a[mex] = mex;
					ans.add(mex+1);
				}
			}
			if(ans.size()>2*n) throw new RuntimeErrorException(null, "err");
	...
	public static int getMex(int[] a) {
		...
		for(int i = 0;;i++) if(!v.contains(i)) return i;
	}
