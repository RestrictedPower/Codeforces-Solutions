...
/*
    Solution Created: 19:27:31 21/07/2020
...
public class Main {
	public static void solve(Reader in, Writer out) {
		/*
		RevString rev = new RevString("110".toCharArray());
		Util.dbg(rev.getFirstChar()+" "+rev.getLastChar());
		rev.makeOperation();
		Util.dbg(rev.getFirstChar()+" "+rev.getLastChar());
		rev.removeLastChar();
		Util.dbg(rev.getFirstChar()+" "+rev.getLastChar());
		...
		*/
		...
		while (t-->0) {
			...
			char[] a = in.nca();
			char[] b = in.nca();
			RevString rev = new RevString(a);
			for(int i = n-1; i>=0; i--) {
				if(rev.getLastChar()==b[i]) {
					rev.removeLastChar();
					continue;
				}
				if(rev.getFirstChar()==b[i]) {
					ans.add(1);
					rev.reverseFirstChar();
				}
				ans.add(i+1);
				rev.makeOperation();
				rev.removeLastChar();
	...
	public static class RevString{
		...
		public char getFirstChar() {
			//Util.dbg(reversed?a[r]:a[l], reversed,"first", xorChar(reversed?a[r]:a[l], swapped));
			return xorChar(reversed?a[r]:a[l], swapped);
		}
		public char getLastChar() {
			//Util.dbg(reversed?a[l]:a[r], reversed,"last",xorChar(reversed?a[l]:a[r], swapped));
			return xorChar(reversed?a[l]:a[r], swapped);
		}
		...
		public void reverseFirstChar() {
			if(!reversed) {
				if(a[l]=='1') a[l] = '0'; else a[l] = '1';
			}else {
				if(a[r]=='1') a[r] = '0'; else a[r] = '1';
			}
		}
		public char xorChar(char a, boolean b) {
			if(a=='1' && b) return '0';
			if(a=='0' && (!b)) return '0';
			return '1';
		}
