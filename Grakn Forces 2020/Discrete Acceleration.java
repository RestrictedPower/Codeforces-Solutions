...
/*
    Solution Created: 18:29:37 30/09/2020
...
public class Main {
	public static void solve(Reader in, Writer out) {
		...
		while(t-->0) {
			...
			n+=2;
			double d;
			double pL = a[0], pR = a[n-1];
			double ans = 0;
			while(true) {
				double s1 = getSpeedL(pL, a);
				double s2 = getSpeedR(pR, a);
				
				int nextPos1 = nextPosL(pL, a);
				int nextPos2 = nextPosR(pR, a);
				if(nextPos1>nextPos2) break;
				double d1 = a[nextPos1]-pL;
				double d2 = pR-a[nextPos2];
				
				double t1 = d1/s1, t2 = d2/s2;
				double mT = Math.min(t1, t2);
				ans += mT;
				pL += s1*mT;
				pR-= s2*mT;
				if(nextPos1==nextPos2) break;
			...
			d = pR-pL;
			ans+= d/(getSpeedL(pL, a)+getSpeedR(pR, a));
			out.println(ans);
		}
	}
	public static double getSpeedL(double pos, int[] a) {
	...
	public static double getSpeedR(double pos, int[] a) {
		int l = 0, r = a.length-1, ans = -1;
		while(l<=r) {
			int md = (l+r)/2;
			if(a[md]>=pos) {
	...
	public static int nextPosL(double pos, int[] a) {
		int l = 0, r = a.length-1, ans = -1;
		while(l<=r) {
			int md = (l+r)/2;
			if(a[md]>pos) {
	...
	public static int nextPosR(double pos, int[] a) {
		int l = 0, r = a.length-1, ans = -1;
		while(l<=r) {
			int md = (l+r)/2;
			if(a[md]<pos) {
