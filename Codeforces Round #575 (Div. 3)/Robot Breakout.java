...
/*
    Solution Created: 18:23:28 24/07/2019
...
public class Main {
	public static void solve(Reader in, Writer out) {
		...
		while(t-->0) {
			...
			boolean[][] moves = new boolean[n][4];
			for(int i = 0; i<n; i++) {
				cords[i][0] = in.nextInt();
				cords[i][1] = in.nextInt();
				for(int j = 0; j<4; j++) moves[i][j] = in.nextInt()==1;
			}
			int U = 100000;//x
			int D = -100000;//x
			int L = -100000;//y
			...
			for(int i = 0; i<n; i++) {
				if(!moves[i][0]) {
					D = Integer.max(D, cords[i][0]);
				}
				if(!moves[i][1]) {
					R = Integer.min(R, cords[i][1]);
				}
				if(!moves[i][2]) {
					U = Integer.min(U, cords[i][0]);
				}
				if(!moves[i][3]) {
					L = Integer.max(L, cords[i][1]);
				}
			}
			if(U>=D&&L<=R) {
				out.println(1+" "+U+" "+L);
