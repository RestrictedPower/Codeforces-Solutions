...
/*
    Solution Created: 18:47:52 24/07/2019
...
public class Main {
	public static void solve(Reader in, Writer out) {
		...
		while(t-->0) {
			...
			ans = Math.min(ans, check(a, new char[]{'R','G','B'},k));
			ans = Math.min(ans, check(a, new char[]{'G','B','R'},k));
			ans = Math.min(ans, check(a, new char[]{'B','R','G'},k));
			out.println(ans);
		}
	}
	public static int check(char[] a, char[] rgb, int k) {
		int n = a.length;
		int[] pref = new int[n+1];
		pref[0] = 0;
		for(int i = 0; i<n; i++) {
			pref[i+1] = a[i]==rgb[i%3]?0:1;
			pref[i+1] += pref[i];
		}
		//Util.dbg(pref);
		int ans = Integer.MAX_VALUE;
		for(int i = 0; i+k<n+1; i++) {
			ans = Math.min(ans, pref[i+k]-pref[i]);
		}
		return ans;
	}
