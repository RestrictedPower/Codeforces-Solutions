...
/*
    Solution Created: 19:13:32 04/09/2020
...
public class Main {
	public static void solve(Reader in, Writer out) {
		...
		while(t-->0) {
			...
			long[] maxGainFromHere = new long[n];
			maxGainFromHere[n-1] = gain[n-1];
			for(int i = n-2; i>=0; i--) maxGainFromHere[i] = Math.max(maxGainFromHere[i+1], gain[i]);
			for(int i = 0; i<n; i++) {
				int nextIdx = (int) (saveUpto[i]+1);
				if(nextIdx>=n) continue;
				ans = Math.max(ans, gain[i]+maxGainFromHere[nextIdx]);
			}
			out.println(ans);
		}
	}
