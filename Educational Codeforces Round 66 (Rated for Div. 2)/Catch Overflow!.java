...
/*
    Solution Created: 20:51:14 06/06/2019
	...
	public static void main(String[] args) {
		...
		for(int i = 0; i<n&&ans!=-1; i++) {
			...
			if(s.equalsIgnoreCase("add")) {
				ans+=deep;
				if(ans>max) ans = -1;
				if(toBreakLen>0) ans=-1;
			}else if(s.equalsIgnoreCase("end")) {
				if(toBreakLen==0) deep/=prev.pop();
				else toBreakLen--;
			}else {
				long m = in.nextLong();
				if(deep*m>max|toBreakLen>0) toBreakLen++;
				else {
					deep*=m;
					prev.push(m);
				}
		...
		if(ans>max) ans = -1;
		if(ans==-1) System.out.println("OVERFLOW!!!");
