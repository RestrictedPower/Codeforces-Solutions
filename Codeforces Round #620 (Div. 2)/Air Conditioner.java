...
/*
    Solution Created: 18:48:01 15/02/2020
...
public class Main {
	public static void solve(Reader in, Writer out) {
		...
		while(q-->0) {
			...
			int hp = m, lp = m;
			for(int i = 0; i<n; i++) {
				int t = in.nextInt(), l = in.nextInt(), h = in.nextInt();
				int changes = t-curTime;
				if(hp+changes<l||lp-changes>h) sat = false;
				hp = Math.min(h, hp+changes);
				lp = Math.max(l, lp-changes);
				curTime = t;
			}
