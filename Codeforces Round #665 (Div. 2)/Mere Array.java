...
/*
    Solution Created: 18:37:23 21/08/2020
...
public class Main {
	public static void solve(Reader in, Writer out) {
		...
		while(TT-->0) {
			int n = in.nextInt();
			int[] a = in.na(n);
			out.println(can(a)?"YES":"NO");
		}
	}
	public static boolean can(int[] a) {
		int n = a.length;
		int minIdx = 0;
		for(int i = 0; i<n; i++) if(a[i]<a[minIdx]) minIdx = i;
		int min = a[minIdx];
		...
		Collections.sort(moveAble);
		int idx = 0;
		for(int i = 0; i<n; i++) if(canMove[i]) a[i] = moveAble.get(idx++);
