...
import java.util.Collections;import com.sun.javafx.geom.Edge;
...
/*
    Solution Created: 20:24:15 21/08/2020
...
public class Main {
		...
		while(TT-->0) {
			...
			while(m!=n-1) {
				if(m>n-1) {
					...
					a.remove(a.size()-1);
					a.set(a.size()-1, (int) ((((long)a.get(a.size()-1)%mod)*((long)last%mod))%mod));
				}else a.add(1);
				m+=m>n-1?-1:1;
			}
			dfs(0, -1);
			...
			long ans = 0;
			for(int i = 0; i<n-1; i++) ans = (ans%mod + ((passes.get(i)%mod)*(a.get(i)%mod))%mod)%mod;
			out.println(ans);
		}
	}
	public static int dfs(int i, int p) {
		...
		for(Integer next : adj[i]) {
			if(next==p) continue;
			long after = dfs(next, i)+1;
			passes.add((n-after)*after);
			tot+=after;
		}
		return tot;
	}
