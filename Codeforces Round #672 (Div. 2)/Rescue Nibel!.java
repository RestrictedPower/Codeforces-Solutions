...
/*
    Solution Created: 21:08:42 24/09/2020
...
public class Main {
	...
	static class Point implements Comparable<Point>{
		int v;
		boolean on;
		public Point(int v, boolean on) {
			this.v = v;
			this.on = on;
		}
		@Override
		public int compareTo(Point o) {
			if(this.v==o.v && this.on == o.on) return 0;
			if(this.v==o.v) return this.on?-1:1;
			return this.v - o.v;
		}
	}
	public static void solve(Reader in, Writer out) {
			int t = 1;
			Util.initFactorial(1000000, mod);
			int n = in.nextInt(), k = in.nextInt();
			Point[] a = new Point[2*n];
			int idx = 0;
			for(int i = 0; i<n; i++) {
				a[idx++] = new Point(in.nextInt(), true);
				a[idx++] = new Point(in.nextInt(), false);
			}
			Util.sortArray(a);
			int count = 0;
			long ans = 0;
