...
/*
    Solution Created: 13:03:18 06/08/2020
...
public class Main {
	public static void solve(Reader in, Writer out) {
		...
		while (t-- > 0) {
			...
			Util.sortArray(a);
			int ans = 0;
			for(int sum = a[0]+a[1]; sum<=a[n-1]+a[n-2];sum++) ans = Math.max(ans, getTeams(a, sum));
			out.println(ans);
		}
	}
	public static int getTeams(int[] a, int sum) {
		boolean[] used = new boolean[a.length];
		int ans = 0;
		for(int i = 0; i<a.length; i++) {
			if(used[i]) continue;
			for(int j = 0; j<a.length; j++) {
				if(i==j || used[j] || a[i]+a[j]!=sum) continue;
				ans++;
				used[i] = used[j] = true;
				break;
			}
