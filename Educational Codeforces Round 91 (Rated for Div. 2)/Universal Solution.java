...
/*
    Solution Created: 18:39:11 12/07/2020
...
public class Main {
	public static void solve(Reader in, Writer out) {
		...
		while(t-->0) {
			char[] a = in.nca();
			int[] f = new int[3];
			for(char c : a) f[idx(c)]++;
			int idx = 0;
			for(int i = 0; i<3; i++) if(f[i]>f[idx]) idx = i;
			for(int i = 0; i<a.length; i++) out.print(w(idx));
			out.println();
		}
	}
	public static char w(int c) {
		if(c==0) return 'R';
		if(c==1) return 'P';
		return 'S';
	}
