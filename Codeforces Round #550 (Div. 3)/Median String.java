...
/*
    Solution Created: 01:44:30 17/05/2020
...
public class Main {
	public static void solve(Reader in, Writer out) {
		...
		int carry = 0;
		for(int i = n; i>0; i--) {
			int v1 = a[i-1]-'a', v2 = b[i-1]-'a';
			calc[i] = (v1+v2+carry)%26;
			carry = (v1+v2+carry)/26;
		}
		calc[0] = carry;
		char[] ans = new char[n+1];
		for(int i = 0; i<=n; i++) {
			ans[i] = (char) ((calc[i]/2)+'a');
			if(i<n) calc[i+1] += 26*(calc[i]%2);
		}
