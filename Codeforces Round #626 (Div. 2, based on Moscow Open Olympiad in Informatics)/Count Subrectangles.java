...
/*
    Solution Created: 13:25:07 07/03/2020
...
public class Main {
	public static void solve(Reader in, Writer out) {
		...
		for(int i = 1; i*i<=k; i++) if(k%i==0) divs.add(i);
		...
		for (int i = 0; i < m; i++) {
			if (b[i] == 1) {
				...
				if (sz > 0) {
					c.put(sz, c.getOrDefault(sz, 0)+1);
				}
				sz = 0;
			}
		}
		if (sz > 0) c.put(sz, c.getOrDefault(sz, 0)+1);
		long ans = 0;
		int y = 0;
		for (int i = 0; i < n; i++) {
			if(a[i]==0) { y = 0; continue;} else y++; 
			if(i==n-1||a[i+1]==0) {
				for (Integer x : c.keySet()) ans += getRect(y, x, divs,k)*c.get(x);
			}
		}
		
		out.println(ans);
	}
	public static long getRect(int x, int y, ArrayList<Integer> divs, int k) {
		long ans = 0;
		for(Integer i : divs) {
			long la = -1;
			long lb = -1;
			{
				long a = x+1-i, b = y+1-(k/i);
				if(a>0 && b>0) {
					ans += a*b;
					la = a;
					lb = b;
			...
			{
				long a = x+1-(k/i), b = y+1-i;
				if(a>0 && b>0 && la!=a&&lb!=b){
					ans += a*b;
				}
			}
		}
